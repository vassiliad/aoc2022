...
type Valley struct {
	...
	// VV: PeriodTime, y, Bitmask: 0=>, 1=<, 2=v, 3=^
...
func (s *State) EstimateTimeToEnd(v *Valley) {
	...
	s.EstTime = utilities.AbsInt(v.End-x) + (v.Height - y) + s.Time
}
...
func (v *Valley) MayPushState(state State, prev *State) bool {
	state.EstimateTimeToEnd(v)
...
func PartA(v *utilities.Valley) int {
	valley := Valley{
	...
	initial.EstimateTimeToEnd(&valley)

	fmt.Println("Width", valley.Width, "Height", valley.Height)

	valley.MayPushState(initial, nil)

	target := v.Height*v.Width + v.End

	steps := 0

	for valley.Queue.Len() > 0 {
		item := heap.Pop(&valley.Queue).(*utilities.Item)
		state := item.Value.(State)

		future := state

		if state.Pos == target {
			// neighbours := valley.Neighbours(&state)
			// fmt.Println("------------ Priority", state.EstTime, "next", valley.PrettyNeighbours(neighbours))
			// solutions := ""
			// s := state
			// for {
			// 	solutions = valley.Draw(s.Pos, s.Time, false) + solutions
			// 	if prev, ok := valley.Prev[s]; ok {
			// 		s = prev
			// 	} else {
			// 		fmt.Println("All Steps\n", solutions)
			// 		break
			// 	}
			// }
			return state.Time
		...
		if _, ok := valley.Visited[state.Memo]; ok {
			continue
		}

		valley.Visited[state.Memo] = 1
		...
		n_x := (state.Pos + valley.Width) % valley.Width
		n_y := state.Pos / valley.Width

		if state.Pos < 0 {
			n_y = -1
		}

		if valley.Blizards[future.Time%valley.LCMDimensions][n_y][n_x] == 0 {
			valley.MayPushState(future, &state)
		}

		for _, n := range neighbours {
			next := future
			next.Pos = n
			next.EstimateTimeToEnd(&valley)
			valley.MayPushState(next, &state)
		}
